...
Plugins.prototype.loadPlugins = function (pluginDir) {
  ...
  const sequence = _reorderPlugins(pluginDirs, config, logger);
  ...
  sequence.forEach(function (pluginName) {
    ...
    const pluginHandler = that.loadPlugin({plugin: plugin, pluginName: pluginName});
    pluginHandlers.push(pluginHandler);
  });
...
function _reorderPlugins(dirs, config, logger) {
...
Plugins.prototype.loadPlugin = function (options) {
  ...
  if (_.isFunction(options)) {
    ...
    assert(name, "must have name for function or the options.pluginName");
    plugin = options;
  }else {
