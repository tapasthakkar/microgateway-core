...
module.exports.start = function (plugins, cb) {
  try {
    ...
    if (config.edgemicro.ssl) {
      ...
      const options = {
        key: fs.readFileSync(path.resolve(config.edgemicro.ssl.key), 'utf8'),
        cert: fs.readFileSync(path.resolve(config.edgemicro.ssl.cert), 'utf8')
      }

      if(config.edgemicro.ssl.passphrase) {
        options.passphrase = config.edgemicro.ssl.passphrase;
      }
