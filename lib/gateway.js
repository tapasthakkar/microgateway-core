...
module.exports.start = function (plugins, cb) {
  try {
    ...
    if (config.system.ssl) {
      ...
      const options = {
        passphrase: config.system.ssl.passphrase,
        ca: config.system.ssl.ca,
        ciphers: config.system.ssl.ciphers,
        rejectUnauthorized: config.system.ssl.rejectUnauthorized,
        secureProtocol: config.system.ssl.secureProtocol,
        servername: config.system.ssl.servername
      }

      if(config.system.ssl.key) {
        options.key =  fs.readFileSync(path.resolve(config.system.ssl.key), 'utf8');
      }

      if(config.system.ssl.cert) {
         options.cert = fs.readFileSync(path.resolve(config.system.ssl.cert), 'utf8');
      }

      if(config.system.ssl.pfx) {
        options.pfx = fs.readFileSync(path.resolve(config.system.ssl.pfx), 'utf8');
      }        
