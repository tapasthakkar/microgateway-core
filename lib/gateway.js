...
module.exports.start = function (plugins, cb) {
  try {
    ...
    if (config.edgemicro.ssl) {
      ...
      if(config.edgemicro.ssl.ca) {
        if(Array.isArray(config.edgemicro.ssl.ca)) {
          options.ca = [];
          config.edgemicro.ssl.ca.forEach((cert)=>{
            options.ca.push(fs.readFileSync(path.resolve(cert), 'utf8')); 
          })
        } else {
           options.ca = fs.readFileSync(path.resolve(cert), 'utf8');
        }
      }
