        ...
        if (configService.get().edgemicro.maxHttpHeaderSize && len > configService.get().edgemicro.maxHttpHeaderSize) {
            ...
            async.series(
                ...
                function(err) {
                    ...
                    logger.eventLog({ level: 'info', req: sourceRequest, res: sourceResponse, component: 'plugins-middleware' }, 'No error at source request, creating target request');
                    ...
                    logger.eventLog({ level: 'info', req: sourceRequest, res: sourceResponse, component: 'plugins-middleware' }, 'Initiate request piping');
...
const getPluginHooksForEvent = function(type, options) {
    ...
    const logger = logging.getLogger();
    logger.eventLog({ level: 'info', req: options.sourceRequest, res: options.sourceResponse, component: 'plugins-middleware' }, `preparing plugin map for type '${type}'`);
        ...
        try {
            ...
            if (plugin['on' + handler] && _.isFunction(plugin['on' + handler])) {
                const fx = function(e) {  // }, newData) {  // jshint but allowing arg to be passed without access to it
                    ...
                    logger.eventLog({ level: 'info', req: options.sourceRequest, res: options.sourceResponse, component: 'plugins-middleware' }, `callback executed successfully for handler '${'on' + handler}' of plugin '${plugin.id}'`);
                ...
                logger.eventLog({ level: 'info', req: options.sourceRequest, res: options.sourceResponse, component: 'plugins-middleware' }, `executing plugin handler '${'on' + handler}' for plugin '${plugin.id}'`);
                pluginHandler.apply(null, args);
            } else {
                logger.eventLog({ level: 'info', req: options.sourceRequest, res: options.sourceResponse, component: 'plugins-middleware' }, `plugin '${plugin.id}' does not provide handler function for '${'on' + handler}'`);
