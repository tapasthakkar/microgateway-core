...
module.exports = function(plugins) {
  ...
  return function(sourceRequest, sourceResponse, next) {
    let plugins = pluginMap[sourceResponse.proxy.scope + '_' + sourceResponse.proxy.proxy_name];
...
function handleTargetResponse(targetRequest, targetResponse, options, cb) {
  ...
  if(options.plugins && options.plugins.length) {
    copyOfPluginsReversed = options.plugins.slice().reverse();
  }
...
const getPluginHooksForEvent = function(type, options) {
  ...
  if(plugins && plugins.length) {
    return plugins.map((plugin) => getPluginHookForEvent(plugin, type, options));
  } else {
    return [];
  }
};
    ...
    try {
      ...
      if (plugin['on' + handler] && _.isFunction(plugin['on' + handler])) {
        ...
        if(argsLength === 3) {
          ...
          args = [options.sourceRequest, options.sourceResponse, options.targetRequest, options.targetResponse, data, fx];
        }
