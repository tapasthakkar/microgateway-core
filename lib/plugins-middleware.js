...
const OnDataRequestTransform = require('./ondata_request-transform');
...
function _subscribeToSourceRequestEvents(plugins, promises, sourceRequest, sourceResponse, targetRequest) {
  ...
  var ondata_request_transform = new OnDataRequestTransform({
    pluginHooks: _executePluginHandlers('data', {
      plugins: plugins,
      sourceRequest: sourceRequest,
      sourceResponse: sourceResponse
    }),
  ...
  /*sourceRequest.on('end', function() { <-- readable
...
----- This is all used in the case where buffering happened.  Use this in the other wiring model. ---- TODO TODO
...
function _subscribeToResponseEvents(plugins, sourceRequest, sourceResponse, targetRequest, targetResponse, correlation_id, start, cb) {
  ...
  targetResponse.pipe(OnDataRequestTransform({
    pluginHooks: _executePluginHandlers('data', {
      plugins: plugins,
      sourceRequest: sourceRequest,
      sourceResponse: sourceResponse,
