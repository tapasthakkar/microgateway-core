...
const { matchPath, buildTrie } = require("./pathTrie.js");
...
module.exports = function() {
  ...
  let trie = buildTrie(config.proxies);
    ...
    config.proxies.forEach(function(proxy) {
      ...
      if ((proxy.secure || proxy.secureHttpProxy) && Array.isArray(config.targets)) {
        var filtered = config.targets.filter(function(target) {
          ...
          return (
            (!target.host || target.host === hostname) &&
  ...
  return (req, res, next) => {
    ...
    const proxy = matchPath(reqUrl.pathname, trie);
    ...
    // var target_path = proxy.parsedUrl.pathname + strippedBasePath;
    ...
    var targetURL = url.parse(proxy);
    req.targetPath = proxy + reqUrl.search || "";
    req.targetHostname = targetURL.hostname;
    //add support to override port and secure
    req.targetPort = targetURL.port;
    req.targetSecure = targetURL.secure;
    next();
  };
};
