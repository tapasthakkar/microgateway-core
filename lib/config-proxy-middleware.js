...
module.exports = function() {
    ...
    try {
      const headClone = _.cloneDeep(req.headers);
      Object.keys(headClone).forEach(h => {
        if (utf8.encode(headClone[h]) !== headClone[h]) {
          debug("ILLEGAL CHARACTER");
          console.error("ILLEGAL CHARACTER", h, req.headers);
          return res.end(500);
        }
      });
    } catch (err) {
      console.error("ILLEGAL CHARACTER", req.headers);
      return res.end(500);
    }
