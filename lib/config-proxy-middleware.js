...
module.exports = function () {
  ...
  return (req, res, next) => {
    ...
    const proxy = getProxyFromHostAndBasePath(config.proxies, config.vhosts, reqUrl.hostname, reqUrl.pathname)
...
const getProxyfromBasePath = function (proxies, pathname) {
  ...
  proxies.forEach(function(proxy) {
    // TODO: Matching logic is weird. Should be fixed to be idempotent.
...
const getProxiesForVhost = function (host, hosts, proxies) {
  var hostKey = Object.keys(hosts).map(function(h) {
    var potentialHost = hosts[h];
    if(potentialHost.vhost == host) {
      return h;
    }
  })[0];
...
const getProxyFromHostAndBasePath = function(proxies, hosts, hostname, pathname) {
  var proxiesForVhost = getProxiesForVhost(hostname, hosts, proxies);
