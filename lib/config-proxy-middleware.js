...
module.exports = function () {
  ...
  return (req, res, next) => {
    ...
    const proxy = getProxyFromBasePathAndVhost(config.proxies, config.system.vhosts, reqUrl.pathname, host);
    if (!proxy) {
      ...
      const nomatch = Error(util.format('no match found for path %s on host %s', reqUrl.pathname, host));
...
// this is done brute-force at this time, and also coincidentally is not sound
...
const getProxiesForVhost = function(proxies, hosts, hostname) {
  var hostKey = Object.keys(hosts).map(function(h) {
    var potentialHost = hosts[h];
    if(potentialHost.vhost == hostname) {
      return h;
    }
  })[0];

  if(hostKey) {
    var proxiesForVhost = proxies.filter(function(p) {
      return p.vhost == hostKey;
    });

    return proxiesForVhost;
  } else {
    return [];
  }
...
const getProxyFromBasePathAndVhost = function(proxies, hosts, pathname, hostname) {
  var proxiesForVhost = getProxiesForVhost(proxies, hosts, hostname);
  return getProxyfromBasePath(proxiesForVhost, pathname);
}

module.exports.getProxyFromBasePath = getProxyfromBasePath;
module.exports.getProxyFromBasePathAndVhost = getProxyFromBasePathAndVhost;
