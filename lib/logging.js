...
module.exports.init = function init(stubConfig) {
  ...
  const writeLog = function (level, obj, msg) {
    if (!cluster.isMaster) {
      return proccess.send({ level: level, obj: obj, msg: msg });
    }
    return writeLogRecord(serializeLogRecord(level, logConfig.level, obj, msg));
  }
  logger = {
    info: function (obj, msg) {
      return writeLog('info', logConfig.level, obj, msg);
    },
    warn: function (obj, msg) {
      return writeLog('warn', logConfig.level, obj, msg);
    },
    error: function (obj, msg) {
      return writeLog('error', logConfig.level, obj, msg);
    },
    stats: function (statsInfo, msg) {
      return serializeLogRecord('stats', serializeStats(logConfig.level, { stats: statsInfo }, msg));
    },
  ...
  if (cluster.isMaster) {
    Object.keys(cluster.workers).forEach((id) => {
      cluster.workers[id].on('message', function (msg) {
        logger[msg.level](msg.obj, msg.msg)
      });
    });
  }
