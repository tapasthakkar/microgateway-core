...
const serializeEventLogRecord = (level, record, text, stackTrace) => {
  const Timestamp = new Date().toISOString();
  const hostname = record.h || logger.transactionContextData.host || ''; // by deault print source request hostname, else print from corresponding req object
  let ProcessId = '';
  if (cluster.isMaster) {
    ProcessId = process.pid;
  } else if (cluster.isWorker) {
  ...
  let url = logger.transactionContextData.url; // by deault print source request url
  const APIProxy = url ? url.replace('/','') : '';
  let ClientIp = logger.transactionContextData.ClientIp || '';
  const ClientId = logger.transactionContextData.clientId || '';
  const component = record.component || '';
  let reqMethod = record.m || logger.transactionContextData.method || ''; // by deault print source request method, else print from corresponding req object
  ...
  let correlationId = logger.transactionContextData.correlation_id || '';
