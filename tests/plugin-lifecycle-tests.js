...
describe('test lifecycle events', function() {
  ...
  config = {
    ...
    proxies: [{ base_path: '/v1', secure: false, url: 'http://localhost:' + port }]
  };
  ...
  //     types[type] = true;
  //     console.log('testPlugin.expectedHeaders', testPlugin.expectedHeaders);
  //     assert.equal(_findHeaders(res.headers(), testPlugin.expectedHeaders).length, testPlugin.expectedHeaders.length);
  //     cb();
  //   });
  //   var handler = testPlugin.init();
  //   gateway.addPlugin('test', function test() {
  ...
  it('GET lifecycle all events run', function(done) {
    ...
    gateway.start(function(err) {
      assert(!err, err);
      request(
        { method: 'GET', url: 'http://localhost:' + gatewayPort + '/v1/echo/get', json: true },
        (err, r, body) => {
          assert(!err, body);
